...
export default getProps => ComposedComponent => {
  ...
  class Fridge extends Component {
    ...
    constructor (props, context) {
      ...
      // here if we *always* set the id in the
      // constructor, we ensure that we aren't going
      // to get a 2nd component instance with the wrong id,
      // but when a component is hot-reloaded, everything will
      // re-resolve.
      // sharedState.id = store.getNextId()
      // sharedState.fridgeProps = store.get(sharedState.id) || null

      // id = store.getNextId()
      // const storeState = store.get(id) || null
      // this.state = {props: storeState, resolved: !!storeState}
    }
    ...
    render () {
      ...
      return (this.resolving || !fridgeProps)
        ? null
